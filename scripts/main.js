...
"none";elem.style.webkitUserSelect="none";elem.addEventListener("touchstart",StopPropagation);elem.addEventListener("touchmove",StopPropagation);elem.addEventListener("touchend",StopPropagation);elem.addEventListener("mousedown",StopPropagation);elem.addEventListener("mouseup",StopPropagation);elem.addEventListener("change",()=>this.PostToRuntimeElement("change",elementId,{"files":Array.from(elem.files)}));if(e["id"])elem.id=e["id"];this.UpdateState(elem,e);return elem}UpdateState(elem,e){elem.setAttribute("accept",
e["accept"]);if(e["isMultiSelect"])elem.setAttribute("multiple","");else elem.removeAttribute("multiple")}_Click(elem,e){elem["click"]()}_Clear(elem){elem.value=""}};self.RuntimeInterface.AddDOMHandlerClass(HANDLER_CLASS)};
